...
class A {
    int m =3; // 인스턴스 필드
    static int n = 5; // 스태틱(정적) 필드
}

public class Static1 {
    /* 정적필드의 특징
        - 정적필드는 Heap의 정적영역에 생성됨
        - 생성시점은 main메서드가 실행되기 이전
        - 정적필드의 값은 오직 정적영역에만 존재하고 객체는 그 위치를 참조함
        - 1)정적필드(static field)는 ★전체 애플리케이션에서 유일한 값을 가지고★
        - 2)객체를 생성하지 않아도 읽거나 수정이 가능함 (클래스명.정적필드)

        ex) 같은 계좌를 쓴다고 하면 잔액은 하나의 값만 가져야하므로 static 사용
    ...
    public static void main(String[] args) {
        ...
        System.out.println(a1.n); // 5 이 방법은 권장하지 않음
